/**
 * âš ï¸ IMPORTANT REFERENCE DOCUMENTATION âš ï¸
 * 
...
function formatWhyHtml(rawText) {
  ...
  const items = lines
    .map((line) => {
      // Try to find dash separator (could be " - ", " â€“ ", or " â€” ")
...
if (isPortalRequest) {
  ...
  if (isProjectPortal && Array.isArray(lotsParsedPortal) && lotsParsedPortal.length) {
    for (const lot of lotsParsedPortal) {
      ...
      propertyDescHtmlPortal += `<p><strong>${lotLabel}</strong></p>`;
      propertyDescTextPortal += `Property Description - ${lotLabel}\n`;
      ...
      const bodyCorpQuarterLot = pd && pd.bodyCorpPerQuarter !== undefined ? String(pd.bodyCorpPerQuarter) : (lotProps && lotProps.body_corp__per_quarter !== undefined ? String(lotProps.body_corp__per_quarter) : (lotProps && lotProps.body_corp_per_quarter !== undefined ? String(lotProps.body_corp_per_quarter) : ""));

      const bedsDisplayLot = bedsPrimaryLot && bedsSecondaryLot ? `${bedsPrimaryLot} + ${bedsSecondaryLot}` : (bedsPrimaryLot || bedsSecondaryLot || "");
      const formatBathValueLot = (val) => { if (!val) return ""; return String(val).replace(/point/gi, "."); };
      const bathsDisplayLot = bathPrimaryLot && bathSecondaryLot
        ? `${formatBathValueLot(bathPrimaryLot)} + ${formatBathValueLot(bathSecondaryLot)}`
        : formatBathValueLot(bathPrimaryLot || bathSecondaryLot || "");
      const garageDisplayLot = garagePrimaryLot && garageSecondaryLot ? `${garagePrimaryLot} + ${garageSecondaryLot}` : (garagePrimaryLot || "");
      const carportDisplayLot = carportPrimaryLot && carportSecondaryLot ? `${carportPrimaryLot} + ${carportSecondaryLot}` : (carportPrimaryLot || "");
      const carspaceDisplayLot = carspacePrimaryLot && carspaceSecondaryLot ? `${carspacePrimaryLot} + ${carspaceSecondaryLot}` : (carspacePrimaryLot || "");

      const builtDisplayLot = buildSizeLot ? `${buildSizeLot} sqm approx.` : "";
      const landSizeDisplayLot = landSizeLot ? `${landSizeLot} sqm approx.` : "";
      let titleDisplayLot = "";
      if (titleLot) {
        titleDisplayLot = titleLot
          .replace(/_/g, " ")
          .split(" ")
      ...
      propertyDescHtmlPortal += htmlLine("Car-port", carportDisplayLot);
      propertyDescHtmlPortal += htmlLine("Car-space", carspaceDisplayLot);
      if (landRegistrationLot) propertyDescHtmlPortal += htmlLine("Registration", landRegistrationLot);
      propertyDescHtmlPortal += htmlLine("Built", builtDisplayLot);
      propertyDescHtmlPortal += htmlLine("Land Size", landSizeDisplayLot);
      propertyDescHtmlPortal += htmlLine("Title", titleDisplayLot);
      ...
      if (bedsDisplayLot) propertyDescTextPortal += textLine("Bed", bedsDisplayLot);
      if (bathsDisplayLot) propertyDescTextPortal += textLine("Bath", bathsDisplayLot);
      if (garageDisplayLot) propertyDescTextPortal += textLine("Garage", garageDisplayLot);
      if (carportDisplayLot) propertyDescTextPortal += textLine("Car-port", carportDisplayLot);
      if (carspaceDisplayLot) propertyDescTextPortal += textLine("Car-space", carspaceDisplayLot);
      if (landRegistrationLot) propertyDescTextPortal += textLine("Registration", landRegistrationLot);
      if (builtDisplayLot) propertyDescTextPortal += textLine("Built", builtDisplayLot);
      if (landSizeDisplayLot) propertyDescTextPortal += textLine("Land Size", landSizeDisplayLot);
      if (titleDisplayLot) propertyDescTextPortal += textLine("Title", titleDisplayLot);
    ...
    // Property Description Additional Dialogue (shared, after all lots)
    const propDescDialoguePortal = vPortal("property_description_additional_dialogue");
    if (propDescDialoguePortal) {
      propertyDescHtmlPortal += `<br>`;
      const propDescDialogueHtmlPortal = normaliseNewlines(propDescDialoguePortal)
        .split(/\n+/)
        .map((l) => l.trim())
        .filter(Boolean)
        .join("<br>");
      propertyDescHtmlPortal += `<p>*${propDescDialogueHtmlPortal}</p>`;
    ...
    if (isDualPortal) {
      ...
      if (hasGarageEitherSidePortal) {
        // Either side has garage â†’ show both sides (including 0s)
        ...
        // Neither side has garage AND no carport/carspace â†’ show "0 + 0"
        garageDisplayPortal = "0 + 0";
      }
      // If neither side has garage BUT carport/carspace exists â†’ don't show garage (handled above)
      ...
      if (!hasCarportPrimaryPortal && !hasCarspacePrimaryPortal) {
        // No carport/carspace â†’ always show garage (even if 0)
        garageDisplayPortal = String(garagePrimaryNumPortal);
      } else if (garagePrimaryNumPortal > 0) {
        // Garage has value AND carport/carspace exists â†’ show all
        garageDisplayPortal = String(garagePrimaryNumPortal);
      }
      // If garage=0 and carport/carspace exists â†’ don't show garage
  ...
  // Body Corp Dialogue: for projects, one shared field shown as its own section row
  let bodyCorpDialogueHtmlPortal = "";
  let bodyCorpDialogueTextPortal = "";
  if (isProjectPortal && bodyCorpDescriptionPortal) {
    const bcDescNorm = normaliseNewlines(bodyCorpDescriptionPortal);
    const bcDescHtml = bcDescNorm
      .split(/\n+/)
      .map((l) => l.trim())
      .filter(Boolean)
      .join("<br>");
    bodyCorpDialogueHtmlPortal = `<p>${bcDescHtml}</p>`;
    bodyCorpDialogueTextPortal = `Body Corp Dialogue\n${bcDescNorm}\n\n`;
  }
  ...
  if (
    ...
    if (isProjectPortal && Array.isArray(lotsParsedPortal) && lotsParsedPortal.length) {
      const hasAnySplitPricingPortal = lotsParsedPortal.some((lot) => {
        const lotProps = (lot && lot.properties && typeof lot.properties === "object") ? lot.properties : lot;
      ...
      for (const lot of lotsParsedPortal) {
        ...
        purchaseHtmlPortal += `<p><strong>${lotLabel}</strong></p>`;
        purchaseTextPortal += `Purchase Price - ${lotLabel}\n`;
        ...
        if (isSingleContractLot && totalPriceLot) {
          ...
          purchaseHtmlPortal += `<p><strong>Total Price:</strong> <strong>${totalPriceFormattedLot}</strong></p>`;
          purchaseTextPortal += `Total Price: ${totalPriceFormattedLot}\n`;
          ...
          if (totalPriceToShowLot) {
            ...
            purchaseHtmlPortal += `<p><strong>Price:</strong> <strong>${totalPriceFormattedLot}</strong></p>`;
            purchaseTextPortal += `Price: ${totalPriceFormattedLot}\n`;
          }
          if (landPriceLot) {
            const landPriceFormattedLot = `$${formatNumberWithCommas(landPriceLot)}`;
            purchaseHtmlPortal += `<p style="margin-left: 30px;"><strong>Land:</strong> ${landPriceFormattedLot}</p>`;
            purchaseTextPortal += `    Land: ${landPriceFormattedLot}\n`;
          }
          if (buildPriceLot) {
            const buildPriceFormattedLot = `$${formatNumberWithCommas(buildPriceLot)}`;
            purchaseHtmlPortal += `<p style="margin-left: 30px;"><strong>Build:</strong> ${buildPriceFormattedLot}</p>`;
            purchaseTextPortal += `    Build: ${buildPriceFormattedLot}\n`;
          }
        }
        ...
        if (hasCashbackLot && isCashbackTypeLot) {
          ...
          if (netPriceLot > 0) {
            ...
            purchaseHtmlPortal += `<p><strong>Net Price:</strong> <strong>${netPriceFormattedLot}</strong> when considering the <span style="background-color: #808080; color: #ffffff; padding: 2px 6px; border-radius: 3px; font-weight: bold;">${cashbackFormattedLot} ${cashbackTypeForSentenceLot}</span></p>`;
            purchaseTextPortal += `Net Price: ${netPriceFormattedLot} when considering the ${cashbackFormattedLot} ${cashbackTypeForSentenceLot}\n`;
          }
        }
        ...
        if (hasCashbackLot && isRebateTypeLot) {
          ...
          purchaseHtmlPortal += `<p><strong>Rebate:</strong> <span style="background-color: #808080; color: #ffffff; padding: 2px 6px; border-radius: 3px; font-weight: bold;">${rebateFormattedLot}</span></p>`;
          purchaseTextPortal += textLine("Rebate", rebateFormattedLot);
        }
  ...
  if (hasAnyPortal("occupancy", "occupancy_primary", "occupancy_secondary", "current_rent_primary__per_week", "current_rent_primary_per_week", "current_rent_secondary__per_week", "current_rent_secondary_per_week", "expiry_primary", "expiry_secondary", "yield", "rent_appraisal_primary_from", "rent_appraisal_primary_to", "rent_appraisal_secondary_from", "rent_appraisal_secondary_to", "appraised_yield", "rental_assessment_additional_dialogue")) {
    ...
    if (isProjectPortal && Array.isArray(lotsParsedPortal) && lotsParsedPortal.length) {
      ...
      for (const lot of lotsParsedPortal) {
        ...
        rentalHtmlPortal += `<p><strong>${lotLabel}</strong></p>`;
        rentalTextPortal += `Rental Assessment - ${lotLabel}\n`;
        ...
        if (!appraisalSecondary) {
          if (appraisalPrimary) {
            rentalHtmlPortal += htmlLine("Appraisal", appraisalPrimary);
            rentalTextPortal += textLine("Appraisal", appraisalPrimary);
          }
        } else {
          ...
          if (appraisalTotal) {
            rentalHtmlPortal += `<p>Total: ${appraisalTotal}</p>`;
            rentalTextPortal += `    Total: ${appraisalTotal}\n`;
          }
          if (appraisalPrimary) {
            rentalHtmlPortal += `<p style="margin-left: 30px;">Unit A: ${appraisalPrimary}</p>`;
            rentalTextPortal += `    Unit A: ${appraisalPrimary}\n`;
          }
          if (appraisalSecondary) {
            rentalHtmlPortal += `<p style="margin-left: 30px;">Unit B: ${appraisalSecondary}</p>`;
            rentalTextPortal += `    Unit B: ${appraisalSecondary}\n`;
          }
        }
        if (yieldLot) {
          rentalHtmlPortal += htmlLine("Appraised Yield", yieldLot);
          rentalTextPortal += textLine("Appraised Yield", yieldLot);
        }
  ...
  if (hasAnyPortal("median_price_change__3_months", "median_price_change__1_year", "median_price_change__3_year", "median_price_change__5_year", "median_yield", "median_rent_change__1_year", "rental_population", "vacancy_rate", "market_performance_additional_dialogue")) {
    marketHtmlPortal += htmlLineMarket("Median Price Change â€“ 3 months", mp3mPortal);
    marketHtmlPortal += htmlLineMarket("Median Price Change â€“ 1 year", mp1yPortal);
    marketHtmlPortal += htmlLineMarket("Median Price Change â€“ 3 years", mp3yPortal);
    marketHtmlPortal += htmlLineMarket("Median Price Change â€“ 5 years", mp5yPortal);
    marketHtmlPortal += htmlLineMarket("Median Yield", medianYieldPortal);
    marketHtmlPortal += htmlLineMarket("Median Rent Change â€“ 1 year", rentChange1yPortal);
    ...
    marketTextPortal += textLineMarket("Median Price Change â€“ 3 months", mp3mPortal);
    marketTextPortal += textLineMarket("Median Price Change â€“ 1 year", mp1yPortal);
    marketTextPortal += textLineMarket("Median Price Change â€“ 3 years", mp3yPortal);
    marketTextPortal += textLineMarket("Median Price Change â€“ 5 years", mp5yPortal);
    marketTextPortal += textLineMarket("Median Yield", medianYieldPortal);
    marketTextPortal += textLineMarket("Median Rent Change â€“ 1 year", rentChange1yPortal);
  ...
  const propertySectionsHtml = `
                ...
                ${sectionRow("Body Corp Dialogue", bodyCorpDialogueHtmlPortal)}
...
<head>
<meta charset="UTF-8">
<title>${escapeHtml(isProjectPortal ? (projectAddressPortal ? `Property Review (Project): ${projectAddressPortal.toUpperCase()}` : "Property Review (Project)") : `Property Review: ${propertyAddressPortal || propertyAddress}`)}</title>
<style>
  body {
    margin: 0;
    padding: 0;
...
<body>
  ...
  <table class="outer-table" role="presentation" cellpadding="0" cellspacing="0" style="margin-top: 10px;">
    ...
    return {
      to: toEmail,
      subject: isProjectPortal
        ? (() => {
            const projectTypePortal = isHAndLPortal ? "H&L Project" : (isSMSFPortal ? "SMSF Project" : "Project");
            const projectAddrRaw = projectAddressPortal || propertyAddressPortal || "";
            const projectAddrClean = String(projectAddrRaw)
              .trim()
              .replace(/^\s*(lot|unit)\s*[^,]+,\s*/i, "")
              .replace(/^\s*(lot|unit)\s+\S+\s+/i, "");
            return projectAddrClean ? `Property Review (${projectTypePortal}): ${projectAddrClean.toUpperCase()}` : `Property Review (${projectTypePortal})`;
          })()
        : `Property Review: ${propertyAddressPortal || propertyAddress}`,
      htmlBody: htmlBody,
      textBody: textBody,
      baEmail: baEmail,
      baName: baName,
...
// STEP 4 â€“ Build final email: subject + HTML + text
...
if (baApproved === "approved") {
  ...
  subjectPrefix = "BA AUTO SEND â€“ ";
} else if (packagerApproved === "approved") {
  subjectPrefix = "QA TO VERIFY â€“ ";
} else {
  subjectPrefix = "PACKAGER TO CONFIRM â€“ ";
}
...
if (isProject && propertyAddress) {
  displayAddress = String(propertyAddress)
    .trim()
    .replace(/^\s*(lot|unit)\s*[^,]+,\s*/i, "")
    .replace(/^\s*(lot|unit)\s+\S+\s+/i, "");
}
...
// Filter + dedupe lots for this project only (matches portal logic)
const projectIdentifierForFilter = projectIdentifier || (input && input.project_identifier ? String(input.project_identifier).trim() : "");
if (projectIdentifierForFilter) {
  lotsParsed = lotsParsed.filter((lot) => {
    const lotProps = (lot && lot.properties && typeof lot.properties === "object") ? lot.properties : lot;
    const pid = lotProps && (lotProps.project_identifier || lotProps.projectIdentifier) ? String(lotProps.project_identifier || lotProps.projectIdentifier).trim() : "";
    return pid === projectIdentifierForFilter;
  });
}
{
  const seen = new Set();
...
if (
  ...
  lotsParsed.forEach((lot) => {
    ...
    propertyDescHtml += `<p><strong>${lotLabel}</strong></p>`;
    propertyDescText += `Property Description - ${lotLabel}\n`;
...
// Body Corp Dialogue: for projects, one shared field shown as its own section row
let bodyCorpDialogueHtml = "";
let bodyCorpDialogueText = "";
if (isProject && bodyCorpDescription) {
  const bcDescNorm = normaliseNewlines(bodyCorpDescription);
  const bcDescHtmlContent = bcDescNorm
    .split(/\n+/)
    .map((l) => l.trim())
    .filter(Boolean)
    .join("<br>");
  bodyCorpDialogueHtml = `<p>${bcDescHtmlContent}</p>`;
  bodyCorpDialogueText = `Body Corp Dialogue\n${bcDescNorm}\n\n`;
}
...
if (
  ...
  if (isProject) {
    ...
    projectLots.forEach((lot) => {
      ...
      purchaseHtml += `<p><strong>${lotLabel}</strong></p>`;
      purchaseText += `Purchase Price - ${lotLabel}\n`;
...
if (
  ...
  if (isProject) {
    ...
    projectLots.forEach((lot) => {
      ...
      rentalHtml += `<p><strong>${lotLabel}</strong></p>`;
      rentalText += `Rental Assessment - ${lotLabel}\n`;
...
if (
  ...
  marketHtml += htmlLineMarket("Median Price Change â€“ 3 months", mp3m);
  marketHtml += htmlLineMarket("Median Price Change â€“ 1 year", mp1y);
  marketHtml += htmlLineMarket("Median Price Change â€“ 3 years", mp3y);
  marketHtml += htmlLineMarket("Median Price Change â€“ 5 years", mp5y);
  marketHtml += htmlLineMarket("Median Yield", medianYield);
  marketHtml += htmlLineMarket("Median Rent Change â€“ 1 year", rentChange1y);
  ...
  marketText += textLineMarket("Median Price Change â€“ 3 months", mp3m);
  marketText += textLineMarket("Median Price Change â€“ 1 year", mp1y);
  marketText += textLineMarket("Median Price Change â€“ 3 years", mp3y);
  marketText += textLineMarket("Median Price Change â€“ 5 years", mp5y);
  marketText += textLineMarket("Median Yield", medianYield);
  marketText += textLineMarket("Median Rent Change â€“ 1 year", rentChange1y);
          ...
          <tr>
            <td>
              <table class="main-table" role="presentation" cellpadding="0" cellspacing="0">
                ...
                ${sectionRow("Body Corp Dialogue", bodyCorpDialogueHtml)}
