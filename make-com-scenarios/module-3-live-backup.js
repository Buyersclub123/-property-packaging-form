...
if (isPortalRequest) {
  ...
  if (hasAnyPortal("occupancy", "occupancy_primary", "occupancy_secondary", "current_rent_primary__per_week", "current_rent_primary_per_week", "current_rent_secondary__per_week", "current_rent_secondary_per_week", "expiry_primary", "expiry_secondary", "yield", "rent_appraisal_primary_from", "rent_appraisal_primary_to", "rent_appraisal_secondary_from", "rent_appraisal_secondary_to", "appraised_yield", "rental_assessment_additional_dialogue")) {
    ...
    if (!isDualPortal) {
      ...
      if (isEstablishedPortal) {
        // Derive property-level occupancy from per-unit values (matching tri-plus pattern)
        const occPrimaryValPortal = occupancyPrimaryPortal ? occupancyPrimaryPortal.toLowerCase() : "";
        const occSecondaryValPortal = occupancySecondaryPortal ? occupancySecondaryPortal.toLowerCase() : "";
        let derivedOccupancyPortal = occupancyPortal || "";
        if (!derivedOccupancyPortal && occPrimaryValPortal && occSecondaryValPortal) {
          if (occPrimaryValPortal === occSecondaryValPortal) {
            derivedOccupancyPortal = occupancyPrimaryPortal; // Both same — use that value
          } else {
            derivedOccupancyPortal = "partially_tenanted";
          }
        } else if (!derivedOccupancyPortal && (occPrimaryValPortal || occSecondaryValPortal)) {
          derivedOccupancyPortal = occupancyPrimaryPortal || occupancySecondaryPortal || "";
        }
        rentalHtmlPortal += htmlLine("Occupancy", formatOccupancy(derivedOccupancyPortal));
      ...
      if (isEstablishedPortal) {
        // Derive property-level occupancy from per-unit values (matching tri-plus pattern)
        const occPrimaryValTextPortal = occupancyPrimaryPortal ? occupancyPrimaryPortal.toLowerCase() : "";
        const occSecondaryValTextPortal = occupancySecondaryPortal ? occupancySecondaryPortal.toLowerCase() : "";
        let derivedOccupancyTextPortal = occupancyPortal || "";
        if (!derivedOccupancyTextPortal && occPrimaryValTextPortal && occSecondaryValTextPortal) {
          if (occPrimaryValTextPortal === occSecondaryValTextPortal) {
            derivedOccupancyTextPortal = occupancyPrimaryPortal;
          } else {
            derivedOccupancyTextPortal = "partially_tenanted";
          }
        } else if (!derivedOccupancyTextPortal && (occPrimaryValTextPortal || occSecondaryValTextPortal)) {
          derivedOccupancyTextPortal = occupancyPrimaryPortal || occupancySecondaryPortal || "";
        }
        rentalTextPortal += textLine("Occupancy", formatOccupancy(derivedOccupancyTextPortal));
        if (occupancyPrimaryPortal) {
...
if (
  ...
  if (isProject) {
    ...
    if (isEstablished) {
      // Derive property-level occupancy from per-unit values (matching tri-plus pattern)
      const occPrimaryVal = occupancyPrimary ? occupancyPrimary.toLowerCase() : "";
      const occSecondaryVal = occupancySecondary ? occupancySecondary.toLowerCase() : "";
      let derivedOccupancy = occupancy || "";
      if (!derivedOccupancy && occPrimaryVal && occSecondaryVal) {
        if (occPrimaryVal === occSecondaryVal) {
          derivedOccupancy = occupancyPrimary; // Both same — use that value
        } else {
          derivedOccupancy = "partially_tenanted";
        }
      } else if (!derivedOccupancy && (occPrimaryVal || occSecondaryVal)) {
        derivedOccupancy = occupancyPrimary || occupancySecondary || "";
      }
      const displayOccupancy = formatOccupancy(derivedOccupancy);
      rentalHtml += htmlLine("Occupancy", displayOccupancy);
      if (occupancyPrimary) {
    ...
    if (isEstablished) {
      // Derive property-level occupancy from per-unit values (matching tri-plus pattern)
      const occPrimaryValText = occupancyPrimary ? occupancyPrimary.toLowerCase() : "";
      const occSecondaryValText = occupancySecondary ? occupancySecondary.toLowerCase() : "";
      let derivedOccupancyText = occupancy || "";
      if (!derivedOccupancyText && occPrimaryValText && occSecondaryValText) {
        if (occPrimaryValText === occSecondaryValText) {
          derivedOccupancyText = occupancyPrimary;
        } else {
          derivedOccupancyText = "partially_tenanted";
        }
      } else if (!derivedOccupancyText && (occPrimaryValText || occSecondaryValText)) {
        derivedOccupancyText = occupancyPrimary || occupancySecondary || "";
      }
      rentalText += textLine("Occupancy", formatOccupancy(derivedOccupancyText));
      if (occupancyPrimary) {
        rentalText += `    Unit A: ${formatOccupancy(occupancyPrimary)}\n`;
      }
      if (occupancySecondary) {
