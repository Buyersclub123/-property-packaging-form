...
export async function POST(request: Request) {
  try {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      } else if (imageData.type.includes('jpeg') || imageData.type.includes('jpg') || imageData.type.includes('webp')) {
        // pdf-lib can handle JPEG, and we'll convert webp to jpeg if needed
        try {
          embeddedImage = await pdfDoc.embedJpg(imageBytes);
        } catch (error) {
          // If it fails, try as PNG (webp might need conversion, but let's try)
          embeddedImage = await pdfDoc.embedPng(imageBytes);
        }
      } else {
        // Default to JPEG
      ...
      if (i % imagesPerPage === 0) {
        ...
        // Add property address header on first page only
        if (i === 0) {
    ...
    if (SHARED_DRIVE_ID) {
      permissionOptions.driveId = SHARED_DRIVE_ID;
    }
