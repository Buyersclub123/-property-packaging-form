/**
 * Ã¢Å¡Â Ã¯Â¸Â IMPORTANT REFERENCE DOCUMENTATION Ã¢Å¡Â Ã¯Â¸Â
 * 
...
function formatWhyHtml(rawText) {
  ...
  const items = lines
    .map((line) => {
      // Try to find dash separator (could be " - ", " Ã¢â‚¬â€œ ", or " Ã¢â‚¬â€ ")
      const dashRegex = /[\s]+[-Ã¢â‚¬â€œÃ¢â‚¬â€][\s]+/;
...
function formatInvestmentHighlightsHtml(rawText) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const explicitBullet = /^[-Ã¢â‚¬Â¢]/.test(line);
    const bulletText = explicitBullet ? line.replace(/^[-Ã¢â‚¬Â¢]\s*/, "").trim() : line;
...
if (isPortalRequest) {
  ...
  if (isProjectPortal && Array.isArray(lotsParsedPortal) && lotsParsedPortal.length) {
    for (const lot of lotsParsedPortal) {
      ...
      if (builtDisplayLot) propertyDescHtmlPortal += htmlLine("Built", builtDisplayLot);
      if (landSizeDisplayLot) propertyDescHtmlPortal += htmlLine("Land Size", landSizeDisplayLot);
      if (titleDisplayLot) propertyDescHtmlPortal += htmlLine("Title", titleDisplayLot);
    ...
    const propDescDialoguePortalProject = vPortal("property_description_additional_dialogue");
    if (propDescDialoguePortalProject) {
      propertyDescHtmlPortal += `<br>`;
      const propDescDialogueHtmlPP = normaliseNewlines(propDescDialoguePortalProject)
        .split(/\n+/)
        .map((l) => l.trim())
        .filter(Boolean)
        .join("<br>");
      propertyDescHtmlPortal += `<p>*${propDescDialogueHtmlPP}</p>`;
      propertyDescTextPortal += `\n*${propDescDialoguePortalProject}\n`;
    }
    propertyDescTextPortal += "\n";
    ...
    if (isDualPortal) {
      ...
      if (hasGarageEitherSidePortal) {
        // Either side has garage Ã¢â€ â€™ show both sides (including 0s)
        ...
        // Neither side has garage AND no carport/carspace Ã¢â€ â€™ show "0 + 0"
        garageDisplayPortal = "0 + 0";
      }
      // If neither side has garage BUT carport/carspace exists Ã¢â€ â€™ don't show garage (handled above)
      ...
      if (!hasCarportPrimaryPortal && !hasCarspacePrimaryPortal) {
        // No carport/carspace Ã¢â€ â€™ always show garage (even if 0)
        garageDisplayPortal = String(garagePrimaryNumPortal);
      } else if (garagePrimaryNumPortal > 0) {
        // Garage has value AND carport/carspace exists Ã¢â€ â€™ show all
        garageDisplayPortal = String(garagePrimaryNumPortal);
      }
      // If garage=0 and carport/carspace exists Ã¢â€ â€™ don't show garage
  ...
  if (hasAnyPortal("median_price_change__3_months", "median_price_change__1_year", "median_price_change__3_year", "median_price_change__5_year", "median_yield", "median_rent_change__1_year", "rental_population", "vacancy_rate", "market_performance_additional_dialogue")) {
    marketHtmlPortal += htmlLineMarket("Median Price Change Ã¢â‚¬â€œ 3 months", mp3mPortal);
    marketHtmlPortal += htmlLineMarket("Median Price Change Ã¢â‚¬â€œ 1 year", mp1yPortal);
    marketHtmlPortal += htmlLineMarket("Median Price Change Ã¢â‚¬â€œ 3 years", mp3yPortal);
    marketHtmlPortal += htmlLineMarket("Median Price Change Ã¢â‚¬â€œ 5 years", mp5yPortal);
    marketHtmlPortal += htmlLineMarket("Median Yield", medianYieldPortal);
    marketHtmlPortal += htmlLineMarket("Median Rent Change Ã¢â‚¬â€œ 1 year", rentChange1yPortal);
    ...
    marketTextPortal += textLineMarket("Median Price Change Ã¢â‚¬â€œ 3 months", mp3mPortal);
    marketTextPortal += textLineMarket("Median Price Change Ã¢â‚¬â€œ 1 year", mp1yPortal);
    marketTextPortal += textLineMarket("Median Price Change Ã¢â‚¬â€œ 3 years", mp3yPortal);
    marketTextPortal += textLineMarket("Median Price Change Ã¢â‚¬â€œ 5 years", mp5yPortal);
    marketTextPortal += textLineMarket("Median Yield", medianYieldPortal);
    marketTextPortal += textLineMarket("Median Rent Change Ã¢â‚¬â€œ 1 year", rentChange1yPortal);
...
// STEP 4 Ã¢â‚¬â€œ Build final email: subject + HTML + text
...
if (baApproved === "approved") {
  ...
  subjectPrefix = "BA AUTO SEND Ã¢â‚¬â€œ ";
} else if (packagerApproved === "approved") {
  subjectPrefix = "QA TO VERIFY Ã¢â‚¬â€œ ";
} else {
  subjectPrefix = "PACKAGER TO CONFIRM Ã¢â‚¬â€œ ";
}
...
// Filter + dedupe lots for this project only (match portal logic)
const projectIdentifierNonPortal = v("project_identifier") || (input && input.project_identifier ? String(input.project_identifier).trim() : "");
if (projectIdentifierNonPortal) {
  lotsParsed = lotsParsed.filter((lot) => {
    const lotProps = (lot && lot.properties && typeof lot.properties === "object") ? lot.properties : lot;
    const pid = lotProps && (lotProps.project_identifier || lotProps.projectIdentifier) ? String(lotProps.project_identifier || lotProps.projectIdentifier).trim() : "";
    return pid === projectIdentifierNonPortal;
  });
}
{
  const seen = new Set();
...
if (
  ...
  marketHtml += htmlLineMarket("Median Price Change Ã¢â‚¬â€œ 3 months", mp3m);
  marketHtml += htmlLineMarket("Median Price Change Ã¢â‚¬â€œ 1 year", mp1y);
  marketHtml += htmlLineMarket("Median Price Change Ã¢â‚¬â€œ 3 years", mp3y);
  marketHtml += htmlLineMarket("Median Price Change Ã¢â‚¬â€œ 5 years", mp5y);
  marketHtml += htmlLineMarket("Median Yield", medianYield);
  marketHtml += htmlLineMarket("Median Rent Change Ã¢â‚¬â€œ 1 year", rentChange1y);
  ...
  marketText += textLineMarket("Median Price Change Ã¢â‚¬â€œ 3 months", mp3m);
  marketText += textLineMarket("Median Price Change Ã¢â‚¬â€œ 1 year", mp1y);
  marketText += textLineMarket("Median Price Change Ã¢â‚¬â€œ 3 years", mp3y);
  marketText += textLineMarket("Median Price Change Ã¢â‚¬â€œ 5 years", mp5y);
  marketText += textLineMarket("Median Yield", medianYield);
  marketText += textLineMarket("Median Rent Change Ã¢â‚¬â€œ 1 year", rentChange1y);
