...
# Read old deal sheet
deal_sheet = {}
with open(r'C:\Users\User\Downloads\Deal Sheet 2026 - Deal List (1).csv', 'r', encoding='utf-8-sig') as f:
    reader = csv.reader(f)
    headers = next(reader)
    status_idx = None
    record_id_idx = None
...
# Read GHL extract
ghl = {}
with open(r'C:\Users\User\Downloads\records (49).csv', 'r', encoding='utf-8-sig') as f:
    reader = csv.reader(f)
...
# Find genuine mismatches (both have valid statuses OR GHL has garbage)
mismatches = []
for rid, ds_data in deal_sheet.items():
    if rid in ghl:
        ...
        if normalize_status(ds_status) != normalize_status(ghl_status):
            # Only include if deal sheet has a valid status
            if is_valid_status(ds_status):
                mismatches.append({
                    'id': rid,
                    'address': ds_data['address'],
                    'ds_status': ds_status,
                    'ghl_status': ghl_status if is_valid_status(ghl_status) else '(CORRUPT DATA)',
                })
