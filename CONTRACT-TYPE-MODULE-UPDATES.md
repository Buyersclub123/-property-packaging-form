# Contract Type Field - Make.com Module Updates

**Date:** 2025-01-13  
**Purpose:** Add `contract_type` field mapping to Make.com modules
...
**File:** `MODULE-21-FIXED-CODE.js`  
**Add this line after line 92 (after `deal_type`):**

```javascript
contract_type: nullIfEmpty(formData.decisionTree?.contractTypeSimplified),
```

**Location in code:** After `deal_type` line, before `status` line

---

### 2. Module 12 (Route 2 - Project Child Records)
**File:** `ROUTE-2-MODULE-12-CODE-TO-PASTE-FIXED.js`  
**Add this line after line 81 (after `deal_type`):**
...
### 3. Module 22 (Route 2 - Project Child Records Alternative)
**File:** `ROUTE-2-MODULE-22-COMPLETE-CODE.js`  
**Add this line after line 144 (after `deal_type`):**
...
### 4. Module 3 (Email Template) - ✅ Already Done
**File:** `code/MODULE-3-COMPLETE-FOR-MAKE.js`  
**Status:** Already reads `contract_type` from GHL (line 1521)  
**No changes needed** - field will be available once it's in GHL records

---

## Complete Updated Code

### Module 21 - Complete Code (with contract_type added)
...
**For each module, add this ONE line:**

```javascript
contract_type: nullIfEmpty(formData.decisionTree?.contractTypeSimplified),
```

**OR for Route 2 modules (12 & 22):**

```javascript
contract_type: nullIfEmpty(shared_data.decisionTree?.contractTypeSimplified),
```

**Place it:** Right after the `deal_type` line, before the `status` line.
...
After updating:
1. Submit a test property from form
2. Check GHL record - `contract_type` field should have value
3. Check Module 3 email template - should be able to read `contract_type` from GHL
